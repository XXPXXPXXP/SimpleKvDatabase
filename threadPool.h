...
class threadPool {
    ...
    using Task = function<void()>; //定义函数类型，便于
    vector<thread> threadIDs;          //线程池
    queue<Task> tasks;            //任务队列
    mutex taskLocker;                   //任务队列同步锁
    mutex addThreadLocker;               //线程池添加线程同步锁
    condition_variable taskCv;   //条件阻塞，用来控制线程的运行
    atomic<bool> isRuning{true};     //线程池是否正在运行
    atomic<int> idleThreadCount{0};  //空闲线程数量
...
public:
    ...
    /* 使用可变参数模版类 */
    auto addTasks(Func &&f, Args &&... args) -> future<decltype(f(args...))> {
        if (!isRuning)    // stopped
            throw runtime_error("addTasks on ThreadPool is stopped.");
        ...
        {    // 添加任务到队列
            lock_guard<mutex> lock{taskLocker};//对当前块的语句加锁  lock_guard 是 mutex 的 stack 封装类，构造的时候 lock()，析构的时候 unlock()
...
private:
    ...
    void addThread(unsigned short size) {
        ...
        unique_lock<mutex> lockGrow{addThreadLocker}; //自动增长锁
        for (; threadIDs.size() < MAX_WORK_THREAD && size > 0; --size) {   //增加线程数量,但不超过最大线程数
            threadIDs.emplace_back([this] { //工作线程函数
                while (true) //防止销毁线程池的时候工作线程直接停止,此时任务队列可能不为空
                {
                    ...
                    {
                        unique_lock<mutex> lock{taskLocker};

...
void *reader::singleWorker(reader *_this, int targetSockId, int readerFd[2]) {
    uint32_t magic_number;
    while (read(targetSockId, (char *) &magic_number, 4) > 0)//判断socket是否结束
    {
        log(info, "线程读取sock成功", targetSockId);
...
void reader::addSingleTask(int targetSockId, int readerFd[2]) {
    runningSingleModeThreads.emplace_back(singleWorker, this,targetSockId, readerFd);
}
